...
public class BaseActor extends Group
{
    ...
    /**
     * Center camera on this object, while keeping camera's range of view
     * (determined by screen size) completely within bounds.
     *
     * Made specifically for objects of size 60x60 - Example: [Player]
     */
    public void alignCameraForPlayer()
    {
        Camera cam = this.getStage().getCamera();
        Viewport v = this.getStage().getViewport();

        // center camera on actor
        cam.position.set( this.getX() + 30, this.getY() + 30, 0 );
